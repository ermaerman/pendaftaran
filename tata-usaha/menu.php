<div id='cssmenu'>
   <ul>
      <li><a href='tu.php?content=home-tata-usaha'><i class="fa fa-home fa-fw"></i>&nbsp; Home</a></li>
      <li><a href='tu.php?content=tahun-pelajaran'><i class="fa fa-calendar fa-fw"></i>&nbsp; Tahun Pelajaran</a></li>
      <li><a href='tu.php?content=data-calon-murid'><i class="fa fa-clipboard fa-fw"></i>&nbsp; Data Calon Murid</a></li>
       <li class='active has-sub'><a href='#'><i class="fa fa-bars fa-fw"></i>&nbsp; Transaksi Keuangan</a>
         <ul>
            <li><a href='tu.php?content=transaksi-pendaftaran'><span>Pendaftaran</span></a>
            </li>
            <li class='has-sub'><a href='#'><span>Uang Pangkal</span></a>
               <ul>
                  <li><a href='tu.php?content=transaksi-cicilan'><span>Cicilan</span></a></li>
                  <li><a href='tu.php?content=transaksi-lunas'><span>Langsung Lunas</span></a></li>
               </ul>
            </li>
            <li><a href='tu.php?content=daftar-ulang'><span>Daftar Ulang</span></a>
            </li>
            <li><a href='tu.php?content=detail-keuangan'><span>Detail Keuangan</span></a>
            </li>
         </ul>
      </li>
      <li class='active has-sub'><a href='#'><i class="fa fa-bars fa-fw"></i>&nbsp; Update</a>
         <ul>
            <li><a href='tu.php?content=update-tanggal'><span>Update Tanggal Ujian Masuk</span></a></li>
            <li><a href='tu.php?content=update-nilai'><span>Update Hasil Ujian Masuk</span></a></li>
            <li><a href='tu.php?content=update-biaya'><span>Update Informasi Biaya</span></a></li>
         </ul>
      </li>
      <li class='active has-sub'><a href='#'><i class="fa fa-copy fa-fw"></i>&nbsp; Kelas</a>
         <ul>
            <li><a href='tu.php?content=pembagian-kelas'><span>Pembagian Kelas Awal</span></a>
            </li>
            <li><a href='tu.php?content=request-kelas'><span>Request Kelas</span></a>
            </li>
            <li><a href='tu.php?content=naik-kelas'><span>Naik Kelas</span></a>
            </li>
            <li><a href='tu.php?content=lihat-kelas'><span>Kelas</span></a>
            </li>
         </ul>
      </li>
      <li><a href='tu.php?content=data-murid'><i class="fa fa-clone fa-fw"></i>&nbsp; Data Murid</a></li>
      <li><a href='tu.php?content=data-ukuran-baju'><i class="fa fa-clipboard fa-fw"></i>&nbsp; Data Ukuran Baju</a></li>
      <li><a href='tu.php?content=data-buku'><i class="fa fa-clipboard fa-fw"></i>&nbsp; Data LKS</a></li>
      <li><a href='tu.php?content=laporan-keuangan'><i class="fa fa-database fa-fw"></i>&nbsp; Laporan Keuangan</a></li>
      <li class='last'><a onClick="return confirm('Apakah anda ingin keluar?')" href='../config/logout.php'><i class="fa fa-sign-out fa-fw"></i>&nbsp; Logout</a></li>
   </ul>
</div>
