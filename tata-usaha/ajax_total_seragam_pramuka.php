<?php
	...
	$hasil   = mysqli_query($konek, $edit)or die(mysqli_error($konek));
	$data    = mysqli_fetch_array($hasil);  
	$seragam_pramuka = $data['seragam_pramuka'];
	
	$lunas 		= $jumlahlunas * $seragam_pramuka;
	$cicil 		= $jumlahcicil * $seragam_pramuka;

	$totalseragampramuka   = ceil($lunas+$cicil);
	
	
	echo $totalseragampramuka;
